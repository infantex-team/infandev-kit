...
Examples:
  ...
  python scripts/checklist.py . --url http://localhost:3000  # Include performance
    ...
    # Detect agent directory dynamically (default to .agentss if exists, fallback to .agents)
    agent_dir_name = ".agentss" if (project_path / ".agentss").exists() else ".agentss"
    ...
    for name, script_path, required in CORE_CHECKS:
        actual_script_path = Path(script_path.replace(".agentss", agent_dir_name))
