...
| :--- | :--- | :--- |
| **Map Project Scope** | `python .agents/skills/agent-ops/scripts/registry.py map` | Lists all active plugins and capabilities limits. |
| **Lint & Validate** | `python .agents/skills/lint-and-validate/scripts/lint_runner.py .` | Fast static analysis. Required before deployment. |
| **Security Surface** | `python .agents/skills/vulnerability-scanner/scripts/security_scan.py .` | Deep OWASP verification. |
| **E2E Validation** | `python .agents/skills/webapp-testing/scripts/playwright_runner.py` | Full Chromium UI simulation. |
| **UX & A11y Audit** | `python .agents/skills/frontend-design/scripts/ux_audit.py .` | Validates color contrast and ARIA limits. |
...
ls .agents/skills/<skill-name>/scripts/
```
